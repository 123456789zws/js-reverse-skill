...
输出: mode = resume | fresh
  resume → 跳过 GATE-1 完整环境自检，读最新阶段报告续接，直接进 GATE-2
  fresh  → 走完整 GATE-1，通过后用 --write-snapshot 写入/更新快照
...
续接模式只跳过 GATE-1 环境自检，**不跳过** GATE-2 证据门禁和 GATE-3 用户确认。
