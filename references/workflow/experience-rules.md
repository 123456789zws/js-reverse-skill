...
> 本文件是 SKILL.md 经验法则（20 条）的扩展说明，编号与 SKILL.md 完全一致。每条给出背景、示例、反例与注意事项，便于在 Phase 1-5 中查阅。
...
拿到验证码成功样本 URL/参数（trace 或浏览器成功链路的请求）后，**先完整解密再写代码**。成功样本就是服务端接受的"标准答案"：把每个密文字段（d/m/p/f/ext、w、cb 等）逐个解密成明文，核对字段类型、值域、格式；轨迹类字段做逐点 diff 统计（步长序列 / 时间间隔 / 点数-距离关系）后再写生成器。**反例**：只解关注字段就动手写代码，凭猜补其余字段（如把易盾 `m` 写成数字 0 而非空串 `""`），导致所有后续排除实验（replay 真实轨迹/真实 cb/真实 fp）全被这一个错误掩盖，白费大半天。易盾 jigsaw 案例：6 份真实样本 `data.m` 均为空串，写 0 全量 result:false。注意事项：字段解密顺序从"结构已知的简单字段"开始（如 ext 长度/数值类），逐步到"复杂加密字段"（如轨迹/指纹），每个字段解出后先记录明文样本再继续；轨迹统计至少 2 个不同距离的样本，才能得出"点数由时长决定还是距离决定"这类结论。
