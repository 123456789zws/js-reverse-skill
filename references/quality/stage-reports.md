...
> **阶段报告默认不生成**。仅多轮复杂补环境 case、上下文防耗尽检查点触发或用户明确要求时按需生成。快速解题场景跳过阶段报告，只产出 `result/最终项目总结.md`。关键结论随节点落盘（IDENTIFY 结论、WASM 黑盒跑通、body 结构确认、实现方案选定等）不受默认省略限制。
...
- 最终交付检查时，如果阶段报告已启用，应确认 `case/阶段报告/` 中至少存在 `01-需求信息确认.md`，并确认文件名和内容均为 UTF-8 中文正常显示。
