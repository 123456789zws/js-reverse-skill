...
期望：
- 不开始逆向分析。
- 列出缺失的必填字段：目标加密参数名、API URL、请求方法、参数位置、成功请求样本、最终请求 TLS 指纹兼容客户端。取证路径由 EVIDENCE_GATE 自动判定，不要求用户选择。
- 提供信息收集模板。
...
期望：
- 不进入正式流程，要求补充 API URL、请求方法、参数位置、成功请求样本和 TLS 客户端。取证路径由 EVIDENCE_GATE 自动判定，不要求用户选择。
