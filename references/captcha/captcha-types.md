...
- 坐标来源判定（**第一步，必做**）：缺口 x 有 3 种来源——A 接口参数加密带坐标、B 图片像素隐写（如极验 v4 bg 图，SDK 从像素里读 x）、C 纯图像识别（只能看图）。先判来源再选路线：A/B 走逆向取坐标（精确、不需要看图），C 才做图像识别/打码。判定信号与流程见 `gap-coordinate-source.md`。
- 坐标校准：把图片像素映射到 DOM/CSS 像素和滑块轨道像素（DPR/缩放/裁剪/轨道 padding）。
- 行为验证：在授权 QA 场景里检查时间、速度和移动曲线是否符合产品限制。
- 参数分析：把答案（x）和厂商加密/绑定参数分开处理；无论坐标来自 A/B/C 哪条路线，verify 参数加密与轨迹加密都照常逆向（`captcha-motion-encryption.md`）。

优先向用户索要背景图和滑块图，同时保留 load/get 响应原文与 SDK 像素读取证据用于来源判定。如果只有一张合成图，要判断是单图边缘/对比任务，还是 canvas 渲染的拼图。
