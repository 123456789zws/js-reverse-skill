...
- 路径：A 纯算还原（加密参数 w）+ 答案层 ddddocr slide_match（滑块距离，本 skill `references/captcha/captcha-solving-handoff.md` 已集成）
- 框架：不使用
...
- 核心思路：
  ...
  - **滑块距离**：用 `ddddocr.DdddOcr(det=False, ocr=False).slide_match(slice_bytes, bg_bytes)` 计算 target_x，按显示比例换算后填入 userresponse 的 distance 与轨迹；**禁止用随机模拟值**（会导致 ajax.php 返回 `{"success":0,"message":"fail"}`）
...
6. **坑：用随机滑动距离提交，加密参数全部正确仍返回 `{"success":0,"message":"fail"}`** → 正确做法：滑块距离必须由 `ddddocr slide_match(slice, bg)` 计算真实缺口 x；视觉答案正确 ≠ 验证通过，还需配合人类轨迹（`scripts/generate_motion_track.py`）与合理 passtime/imgload。**这不是"视觉解题范畴、不属逆向目标"——本 skill 已把 ddddocr 作为答案层默认能力集成，滑块题型必须接入。**
7. **坑：AES 密钥按 hex decode 成 8 字节当 AES key** → 正确做法：极验 AES key 是 16 字符 hex 字符串，但**直接按 Latin1 取 16 字节**作为 aes-128 key（即字符串本身当字节），不是 hex decode。IV 同理（`"0"*16` 的 Latin1 字节，非 `0x00*16`）。
8. **坑：RSA 公钥直接用 `crypto.createPublicKey({key: {modulus, exponent}})`** → 正确做法：Node.js 不支持裸 modulus/exponent 入参，需手动构造 PKCS#1 DER（`SEQUENCE { INTEGER modulus, INTEGER exponent }`），modulus/exponent 首字节最高位为 1 时前补 `0x00`，再 `createPublicKey({key: der, format:'der', type:'pkcs1'})`。
...
- 素材 URL 从 `get.php` 响应 JSON 的 `bg`/`slice` 字段提取（相对路径，需拼接 `static.geetest.com/` 或 `static.geevisit.com/` 前缀）
- 素材下载用与业务请求一致的 TLS 客户端 + 相同 Session cookie（部分厂商素材 URL 绑 Session）
- 闭环：`ddddocr slide_match → target_x → map_coordinates 换算 → generate_motion_track --distance <x> → track → 填 answer JSON → 加密进 w`
- 校验：`node scripts/check_captcha_answer.js --file answer.json`
